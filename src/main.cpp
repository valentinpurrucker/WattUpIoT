...
Task wifiTask = {0, std::bind(&WifiManager::loop, &wifiManager), 1000ul, false,
                 true};
Task mqttTask = {1, std::bind(&MqttPublisher::loop, &mqttPublisher), 1000ul,
                 false, true};
Task ntpTask = {2, std::bind(&NtpClient::update, &ntpClient), 1000ul, false,
                true};
Task eReaderTask = {3, std::bind(&ElectricMeterReader::update, &eReader), 0ul,
                    false, false};
Task eReaderReadTask = {
    4, std::bind(&ElectricMeterReader::sendDataIfAvailable, &eReader), 10000ul,
    false, true};
Task otaHandleTask = {5, std::bind(&OtaController::loop, &otaController),
                      5000ul, false, true};
Task eReaderWaitTask = {6, std::bind(&ElectricMeterReader::wait, &eReader),
                        5000ul, false, false};
